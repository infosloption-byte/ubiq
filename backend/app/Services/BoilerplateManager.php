        ...
        return [
            'package.json' => json_encode([
                ...
                'devDependencies' => [
                    ...
                    'vite'                => '^5.4.1',
                    // FIX #15: shipped in the base scaffold rather than left
                    // for the AI to add. tailwind.config.js/postcss.config.js
                    // are common AI-authored files (the AI reaches for
                    // Tailwind by default for styling); package.json is a
                    // protected scaffold file the AI can't fully rewrite, so
                    // if Tailwind isn't already here, any project that uses
                    // it crashes on first run with "Cannot find module
                    // 'tailwindcss'" the moment Vite processes CSS. See also
                    // the package.json dependency-merge fix in
                    // ProjectController::scaffold() for packages beyond
                    // Tailwind (react-router-dom, etc).
                    'tailwindcss'         => '^3.4.1',
                    'postcss'             => '^8.4.35',
...
JS,
            ...
            // FIX #15: package.json has "type": "module", so .js config
            // files are ESM by default — matches vite.config.js above.
...
CSS,
            ...
            // FIX #15: Tailwind directives added — content generated by
            // scaffold() elsewhere assumes Tailwind utility classes work
            // (the AI's own generated components use them freely), so
            // without these three lines the classes would resolve to
            // nothing even once tailwindcss itself is installed correctly.
            'src/index.css' => <<<'CSS'
@tailwind base;
@tailwind components;
@tailwind utilities;
        ...
        return [
            'package.json' => json_encode([
                ...
                'devDependencies' => [
                    ...
                    'vite'               => '^5.4.1',
                    // FIX #15: see reactFiles() for the full rationale —
                    // shipped by default so any AI-generated
                    // postcss.config.js/tailwind.config.js has something
                    // real to point at.
                    'tailwindcss'        => '^3.4.1',
                    'postcss'            => '^8.4.35',
                    'autoprefixer'       => '^10.4.17',
...
JS,
            ...
            // FIX #15: package.json has no "type": "module" here (unlike
            // react/vue), so these are CommonJS, not ESM — Next's own
            // tooling expects module.exports for postcss.config.js.
            'postcss.config.js' => <<<'JS'
module.exports = {
  plugins: {
    tailwindcss: {},
...
PROMPT;
    ...
    {
        return <<<PROMPT
Scaffold already exists: package.json (react ^18, vite ^5, tailwindcss ^3 preconfigured), vite.config.js, postcss.config.js, tailwind.config.js, index.html, src/main.jsx, src/App.css, src/index.css

DO NOT REGENERATE: package.json, vite.config.js, postcss.config.js, tailwind.config.js, index.html, src/main.jsx, ubiq.json
...
• Tailwind CSS utility classes are available and preferred for styling — no setup needed
...
PROMPT;
    ...
    {
        return <<<PROMPT
Scaffold already exists: package.json (vue ^3.4, vite ^5, tailwindcss ^3 preconfigured), vite.config.js, postcss.config.js, tailwind.config.js, index.html, src/main.js, src/style.css

DO NOT REGENERATE: package.json, vite.config.js, postcss.config.js, tailwind.config.js, index.html, src/main.js, ubiq.json
...
RULES: Vue 3 Composition API with <script setup> syntax. Tailwind CSS utility classes are available and preferred for styling — no setup needed. Complete working code only.
PROMPT;
    }
    ...
    {
        return <<<PROMPT
Scaffold: Next.js 14 App Router. package.json (tailwindcss ^3 preconfigured), next.config.mjs, postcss.config.js, tailwind.config.js, app/layout.jsx, app/page.jsx, app/globals.css already exist.

DO NOT REGENERATE: package.json, next.config.mjs, postcss.config.js, tailwind.config.js, app/layout.jsx, ubiq.json
...
RULES: App Router (app/ directory). Server Components by default. Add 'use client' only when needed for interactivity. Tailwind CSS utility classes are available and preferred for styling — no setup needed.
PROMPT;
    }

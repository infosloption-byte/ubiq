...
import SourceControlPanel from '../components/panels/SourceControlPanel'; // Make sure this file exists!
...
import { 
  ...
  ArrowPathIcon,
  CloudArrowUpIcon,
  CommandLineIcon // Using this as Git Icon if using Heroicons
...
export default function EditorPage() {
  ...
  const loadProject = async () => {
    try {
      ...
      if (!currentFile && response.data.project.files?.length > 0) {
        // Find README or index file first, else pick first available
        const readme = response.data.project.files.find((f: File) => f.name.toLowerCase().includes('readme'));
        const first = readme || response.data.project.files[0];
    ...
    <Layout>
      <div className="h-full flex flex-col md:flex-row overflow-hidden bg-ubiq-950">
        ...
        <div className="w-12 bg-ubiq-950 border-r border-white/5 flex flex-col items-center py-4 gap-4 shrink-0 z-20">
            ...
            />
            <ActivityIcon 
                icon={<CommandLineIcon className="w-5 h-5"/>} // Git Icon
        ...
        <div className="w-full md:w-64 bg-ubiq-900 border-r border-ubiq-800 flex flex-col shrink-0">
          ...
          {activeSideTab === 'git' && projectId && (
             <SourceControlPanel projectId={Number(projectId)} />
          )}
        ...
        {showNewFileDialog && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
            <div className="bg-ubiq-900 border border-white/10 p-6 rounded-xl w-full max-w-sm shadow-2xl">
...
const ActivityIcon = ({ icon, active, onClick, label }: any) => (
    ...
    >
        {icon}
        {/* Tooltip on hover */}
        <span className="absolute left-full top-1/2 -translate-y-1/2 ml-3 px-2 py-1 bg-black text-white text-[10px] rounded opacity-0 group-hover:opacity-100 whitespace-nowrap pointer-events-none transition-opacity z-50">
            {label}
        </span>
    </button>
);
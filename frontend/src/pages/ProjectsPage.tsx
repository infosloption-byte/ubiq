...
interface Project {
    ...
    updated_at: string;
    created_at: string; // FIX: was missing from the interface but accessed in the template
    files_count?: number;
}
                        ...
                        {filteredProjects.map((project) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${project.source === 'github' ? 'bg-[#24292e] text-white' : 'bg-ubiq-accent/10 text-ubiq-accent'}`}>
                                ...
                                <p className="text-slate-400 text-sm mb-6 line-clamp-2 h-10 mt-1">
                                    {project.description || 'No description provided.'}
                                </p>

                                <div className="flex items-center justify-between text-xs text-slate-500 border-t border-white/5 pt-4">
                                    <div className="flex items-center gap-1.5">
                                        <ClockIcon className="w-3.5 h-3.5" /> Last updated {new Date(project.updated_at).toLocaleDateString()}
                                    </div>
                                    <div>

...
**Sequencing:** F1a (small, immediately closes the "is this really portable" gap, no dependency on anything else) → F1b (the real work — multi-service sandboxes) → F1c (the DB service, meaningless without F1b) → F1d (preview links, can happen in parallel with F1b/c since it only depends on the existing `SandboxRun` tracking, not on multi-service support).

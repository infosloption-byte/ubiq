...
## H — Competitive gap response *(new, 2026-08-16)*

Triaged from `Ubiq_Platform_Analysis_and_Competitive_Gap_Analysis.md` — a full codebase-grounded feature comparison against Replit, Bolt.new, Lovable, v0, and Base44. That doc's §4.2 named nine gaps. This section is the deliberately narrow subset worth actual engineering time; the full triage reasoning (including the five gaps declined on purpose, not dropped by oversight) lives in `PLAN_SYSTEM_TASKS.md`'s Phase H — this section covers only the four that made the cut.

The throughline across all four: in every case, the tempting version of the fix is "build what the competitor built." The version actually planned here is smaller — connect to something that already solves the hard part, the same shape as the AI-provider BYOK pattern that already defines how Ubiq works, rather than growing a second, competing infrastructure business (hosting, a BaaS, an agent platform, a fleet) alongside the actual product.

### H1 — Production deploy via provider integration
The single most consistently-missing capability against every competitor profiled — Bolt, Lovable, v0, and Base44 all treat one-click deploy as core, and Ubiq currently has no answer past an ephemeral dev preview link. The wrong fix is obvious in retrospect: building elastic hosting, per-custom-domain TLS, and abuse handling for arbitrary user-generated apps on a shared box is a different, larger risk than anything F1's preview links ever took on, and it was correctly kept out of scope the first time this was considered. The right-sized fix is a "Deploy to Vercel" flow: OAuth-connect (same shape as the existing GitHub connector), push the project's current files via Vercel's own deploy API, surface the resulting URL. Ubiq orchestrates; Vercel's infrastructure serves the traffic and owns the operational risk.

### H2 — Backend-as-a-service via BYO-Supabase
The reason Lovable/Bolt/Base44 feel instant to non-infra developers isn't the AI — it's that auth, storage, and realtime already exist the moment the app is generated. Building that from scratch is a multi-month undertaking competing against a company that already owns the category. A "Connect Supabase" flow — the user's own project URL and keys, stored with the same encrypted-at-rest pattern `AiKeyController` already uses for AI provider keys — gets most of the practical benefit without Ubiq ever operating a database-as-a-service business. This is also the option most consistent with what Ubiq already is: "bring your own everything," extended one step further, not contradicted.

### H3 — Build/test-check on multi-file proposals
Named gap: Replit Agent 3 autonomously writes and runs its own tests. The literal version of that — a planning-and-self-verification agent loop — pulls directly against G2's own premise, where the differentiator is safety via review, not autonomy via unsupervised execution. Chasing the literal version risks diluting the exact thing the gap analysis identified as a strength. The scoped-down version: after a G2 proposal is generated, run the project's own build/test command in a throwaway container (reusing `ProjectController::runProject()`'s existing boot path, not a new subsystem) and show pass/fail next to the diff. It informs the review decision; it doesn't replace it.

### H4 — Resource-ceiling alerting
The current single-EC2-host ceiling (~2–3 concurrent sandboxes given shared tenancy) is real infrastructure debt, but building elastic multi-region capacity ahead of actual growth would repeat the exact premature-optimization mistake this roadmap avoided when it cut G3/Bucket 3 (the old one) for the identical reason — no real usage signal to size against yet. The right-sized version is cheap insurance, not a project: alert before the ceiling is hit, so it's discovered on a dashboard instead of during an incident. The real scaling work stays deferred until there's an actual trigger.

**Declined on purpose, not dropped by oversight** (full reasoning in `PLAN_SYSTEM_TASKS.md`'s Phase H): real-time collaboration and SSO reaffirm the *old* Bucket 3's removal from this same document on 2026-08-16 — a competitor having the row doesn't change the "no team plan to sell it into" reasoning that removal was based on. Market presence (5–8M users at competitors vs. near zero here) isn't an engineering problem at all and has no line item anywhere in this roadmap on purpose. A visual/no-code editing mode was considered and declined, not just deprioritized — it targets a different buyer than Ubiq's actual differentiators (multi-framework backend choice, developer-facing BYOK) already signal, and building it would dilute positioning rather than close a gap. A mobile app for building on the go stays out for the same reason it always has: narrow precedent, low value at current scale.
...
8. **H1 → H2** (deploy integration, then BYO-Supabase) — both integrations rather than builds, and both close named competitive gaps directly; H1 first since "can I ship this" is the more consistently-named gap of the two. **H3** (build/test-check) and **H4** (resource alerting) can run opportunistically alongside either, since neither depends on H1/H2 or on each other.

The one thing most worth explicitly *not* front-loading: custom domains and full production hosting *as Ubiq's own infrastructure* (deliberately dropped from F1 entirely, not just deferred — see the correction note at the top of F1; H1 revisits the outcome via integration, not by reopening that original scope).

**2026-08-16 note:** this doc previously carried a G3 (self-hosted/on-prem tier) and a different Bucket 3 (real-time collab, SSO, admin analytics) — both removed outright, not deferred. Neither had committed scope beyond a rough sketch, and both were explicitly gated on a precondition (a track record with real paying users; a team/enterprise plan existing to sell into) that doesn't exist yet. If either precondition is actually met later, re-scope fresh at that point rather than resurrecting this version — what "harden for self-hosted" or "SSO" should even mean by then may look different from this sketch.
